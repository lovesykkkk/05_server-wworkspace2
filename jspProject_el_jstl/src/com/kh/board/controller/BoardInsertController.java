...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// enctype이 multipart/form-data로 잘 전송되었을경우 전반적인 내용들이 수행되도록
		if(ServletFileUpload.isMultipartContent(request)) {
			
			//System.out.println("잘 실행되나? 요청 잘되낭");
			
			// 파일 업로드를 위한 라이브러리 : cos.jar (com.oreilly.servlet 의 약자)
			 ...
			 *  => 같은 파일명이 존재할 경우 덮어씌워질 수 있고, 파일명에 한글/특문/띄어쓰기가 포함될 경우 서버에 따라 문제 발생 
			 *  
			 ...
			 *      		하늘.png, 하늘1.png, 하늘2.png // 근데 한글은 안바꿔줌 ㅠㅠ
			 *      	return 수정파일
			 *     } // 카톡도 마찬가지
			 *     
			 *     나만의 방식대로 절대 안겹치도록 rename 할 수 있게 나만의 FileRenamePilicy 클래스 (rename 메소드 정의) 만들기!
			...
			// 3. DB에 기록할 데이터를 뽑아서 vo 에 주섬주섬 담기
			//	> 카테고리 번호, 제목, 내용, 작성자회원번호 뽑아서 Board insert
			//  > 넘어온 첨부파일이 있다면 원본명, 수정명, 저장폴도경로 Attatchment isnert
			...
			if(multipartRequest.getOriginalFileName("upfile") != null) { // 넘어온 첨부파일이 있을 경우
				at = new Attachment();
			...
			if(result > 0) {
				...
				// 실패 = > 에러빨간거
				request.setAttribute("errorMsg", "일반게시글 등록 실패!");
