...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1) 전달값에 한글이 있을 경우 인코등 처리해야됨 (post 방식에만)
		...
		Member loginUser = new MemberService().loginMember(userId,userPwd); // null 혹은 로그인한 맴버객체 담김
		 ...
		 * 2) session : 여기에 담긴 데이터는 내가 직접 지우기 전까지, 세션이 만료(서버멈추거나, 브라우저 종료)되기 전까지
		 ...
		 * 				꺼내고자 한다면 .getAttribute("키") : Object 타입으로 벨류
		 * 			    지우고자 한다면 .removeAttribute("키")
		 * 
		 */
		...
		if(loginUser == null) {
			...
			// 포워딩 방식 : 해당 경로로 선택한 뷰가 보여질 뿐 url은 절대 변경되지 않음 (즉, 요청했을 때의 url이 여전히 남아있음)
			...
			// Servlet 에서는 session에 접근하고자 한다면 우선 세션 객체 얻어와야됨
			HttpSession session = request.getSession();
			session.setAttribute("loginUser",loginUser);
			
			// 1. 포워딩 방식 응답 뷰 출력
			//    해당 선택된 jsp가 보여질 뿐 url에는 여전히 현재 이 서플릿 매핑값이 남아있음
